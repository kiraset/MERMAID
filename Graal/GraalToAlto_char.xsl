...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
        ...
        <xsl:for-each select="$documents">
            ...
            <xsl:variable name="result-filename">
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:value-of select="concat('alto_abbr_glyphs/without-norm/lat-abbr_', $filename, '.xml_alto.xml')"/>
                    </xsl:otherwise>
                </xsl:choose>
    ...
    <xsl:variable name="myTexts" select="document('../texts/qgraal_cm-c.xml')"/>
    ...
    <xsl:template match="tei:zone[@type='line']">
        ...
        <xsl:variable name="myContent">
        	<xsl:apply-templates
        		select="$myTexts/descendant::tei:seg[
        		  not(@type='transposition-target')
        		  and preceding::tei:lb[1]/@xml:id = $myId
        		]
        		|
        		$myTexts/descendant::tei:w[
        		    not(ancestor::tei:seg[@type='transposition-target'])
        			and not(tei:seg)
        			and preceding::tei:lb[1]/@xml:id = $myId
        		]
        		|
        		$myTexts/descendant::tei:pc[
        		  preceding::tei:lb[1]/@xml:id = $myId
        		]
        		|
        		$myTexts/descendant::tei:ptr[
        		@type='transposition-orig'
        		and preceding::tei:lb[1]/@xml:id = $myId
        		]
        		"
        	/>
        ...
        <TextLine ID="{@xml:id}"
            ...
            <String CONTENT="{normalize-space($myContent)}"
                ...
                <xsl:apply-templates select="tei:zone[@type='word']//tei:zone[@type='character']"/>
    ...
    <xsl:template match="tei:zone[@type='character']">
        
        <xsl:variable name="charId" select="substring-after(@xml:id, 'zone-')"/>
        
        <xsl:variable name="char"
            select="$myTexts//tei:c[@xml:id = $charId][1]"/>
    ...
    <xsl:template match="tei:w">
        ...
        <xsl:variable name="next"
            ...
            [not(ancestor-or-self::tei:seg[@type='transposition-target'])]
            [self::tei:w or self::tei:pc]
            )[1]"/>
        
        <xsl:if test="$next/self::tei:w">
            <xsl:text> </xsl:text>
        </xsl:if>
    </xsl:template>
    
    <xsl:template match="tei:pc">
        <xsl:apply-templates select="node()[not(self::tei:reg)]"/>
    	<xsl:text> </xsl:text>
    </xsl:template>
    ...
    <xsl:template match="tei:seg">
    	...
    	<xsl:if test="
        	@part='F'
        	and following::node()[self::tei:w or self::tei:pc][1][not(self::tei:pc)]
        ">
        	<xsl:text> </xsl:text>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="tei:choice">
        <xsl:choose>
            <xsl:when test="$expansion">
                <xsl:apply-templates select="tei:sic | tei:expan | tei:reg"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:apply-templates select="tei:sic | tei:abbr | tei:orig"/>
            </xsl:otherwise>
        </xsl:choose>
    ...
    <xsl:template match="tei:ptr[@type='transposition-orig']">
        ...
        <xsl:variable name="words"
            select="$myTexts//tei:seg[@xml:id = $targetId]//tei:w
            | $myTexts//tei:seg[@xml:id = $targetId]//tei:pc"/>
    ...
    <xsl:template name="next-visible-token">
        <xsl:param name="node"/>
    ...
    <xsl:template match="tei:choice/tei:corr//tei:*"/>
